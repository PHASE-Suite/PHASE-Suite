...
* **Parallel Mismatch Extraction**: The script iterates over each generated chunk file. Inside the loop, for each chunk and for both `fwd` and `rev` strands, a subshell is launched in the background (`&`), enabling parallel execution.
    ...
    * `awk -v st="$st" '...'`: This `awk` script processes the `sam2tsv` output to identify mismatches. It filters for lines where the read name ends with `/fwd` or `/rev` and the CIGAR operation is a mismatch. It checks if the read base is different from the reference base (`alt != ref`) and the reference is not 'N'. It then constructs a unique key for each mismatch: `readname:chromosome:reference_position:reference_base:alternative_base`.
